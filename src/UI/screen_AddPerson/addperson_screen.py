...
class AddPerson(Screen):
	...
	def add_person(self):
		...
		# checking incorrect name
			...
			try:
				if not os.path.isdir(DatasetConfig.folder_persons_data):
					os.mkdir(DatasetConfig.folder_persons_data)
	...
	def cropping(self):
		...
		if not path.exists(DatasetConfig.path_temp):
	...
	def face_detection_backup(self):
		...
		if self.toggle:
			image_path = self.ids.preview_photo.source
			filename = image_path.split("/")[-1]
			temp_dir = f"{DatasetConfig.path_temp}\\{filename}"
			...
			if self.ids.preview_photo.source != '' and not path.exists(temp_dir):
				...
				if len(rects) > 0:
					if not os.path.isdir(DatasetConfig.path_temp):
						os.mkdir(DatasetConfig.path_temp)
